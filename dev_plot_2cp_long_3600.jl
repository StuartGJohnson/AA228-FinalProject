...
pomdp2 = deserialize("pomdp_2cp_3600.jls")
println(length(pomdp2))
policy2 = load_policy(pomdp2, "policy_2cp_3600.out")
...
for j in 1:1000
    ...
    Random.seed!(j)
    if j <= num_gifs
        sim = GifSimulator(filename="policy_2cp_3600x2_$j.gif", max_steps=40)
...
matwrite("policy_2cp_3600x2_hist.mat", Dict("array_hist" => array_hist))
df = DataFrame(mapchecks=vec_samples, steps=vec_steps, reward=vec_reward, belief=vec_belief)
CSV.write("policy_2cp_3600x2_stats.csv", df)